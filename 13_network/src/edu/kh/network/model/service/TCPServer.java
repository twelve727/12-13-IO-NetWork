...
public class TCPServer {
	...
	public void serverStart() {
		// 1. 서버의 포트번호 정함
		int port = 8500; // port 0~65535 사이로 지정 가능
						// 단, 1023번 이하는 이미 사용중인 경우가 많아 제외
		...
		// (B)buffer : 읽고 쓰기 때 효능향상용
		
		OutputStream os = null; // 서버 <-> 클라이언트 [출력용] 스트림 변수
		PrintWriter pw = null; // 출력용 보조 스트림 변수
		// (P)buffer : 읽고 쓰기 때 효능향상용
		
		// BufferedWriter br = null; // 여기선 PrintWriter 사용
		// PrintWriter에는 println() print printf 등 다양한 출력함수를 제공함으로써 파일 출력을 편하게 해준다.
		
		try {
			// 2. 서버용 소켓 객체 생성
			serverSocket = new ServerSocket(port); // 포트번호 매개변수 전달, 서버용 소켓을 생성하여 포트(8500)와 결합
			
			// 3. 클라이언트 쪽에서 접속 요청이 오길 기다림
			
			// 서버용 소켓은 생성되면 클라이언트 요청이 오기 전 까지
			...
			// 6. 보조 스트림을 통해 성능 개선
			// 아까 만들어둔 BufferedReader
			
			br = new BufferedReader( new InputStreamReader(is) );
			// InputStreamReader : 바이트기반 스트림 <-> 문자기반 스트림 연결에 사용
			
			pw = new PrintWriter(os); // 따로 필요 X
			
			// 7. 스트림을 통해 읽고 쓰기
			pw.println("[서버 접속 성공]");
			pw.flush(); // 스트림에 있는 내용을 모두 밀어냄
			// close()도 밀어내는게 가능하지만, 굳이 flush()를 사용하는 까닭
			...
			// 8. 통신 종료
			// 사용한 스트림, 소켓 자원을 반환(close)
			
			// io Exception 오류, try catch문으로 다시 잡아주기
			try {
